...
const STATUS_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  ...
  no_data: { bg: 'bg-gray-700/30', text: 'text-gray-500', label: '—' },
}
...
export default function KpiView() {
  ...
  // Q全体の月別入力状況（データ入力済み月数）
  const filledMonths = exec
    ? exec.kpis.reduce((count, kpi) => {
        return count + kpi.monthlyProgress.filter(m => m.actual !== null).length
      }, 0)
    : 0
  const totalMonthSlots = exec ? exec.kpis.length * exec.kpis[0]?.monthlyProgress.length : 0
  const dataCompletion = totalMonthSlots > 0 ? Math.round(filledMonths / totalMonthSlots * 100) : 0

  // Qの進行度合い（1Q = 10,11,12月 → 現在月が何月目か）
  const qMonths = exec?.kpis[0]?.monthlyProgress.map(m => m.month) ?? []
  const qMonthIndex = qMonths.indexOf(data.currentMonth)
  const qProgressLabel = qMonthIndex >= 0
    ? `${qMonthIndex + 1}ヶ月目 / 3ヶ月中`
    : ''
      ...
      <div className="bg-gray-800 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right text-[10px] text-gray-500">
            データ入力率 <span className="text-cyan-400 font-bold">{dataCompletion}%</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {data.executives.map(e => {
            // Q初期は評価ランクの代わりに進捗表示
            const hasEnoughData = e.kpis.some(k => k.monthlyProgress.some(m => m.actual !== null))
            return (
              <button
                key={e.id}
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  {hasEnoughData ? (
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-700 text-gray-400">集計中</span>
                  )}
                </div>
                ...
                {hasEnoughData ? (
                  ...
                  <div className="text-[10px] text-gray-500 mt-1">データ入力待ち</div>
                )}
              </button>
            )
          })}
      ...
      {exec && (
        <>
          ...
          <div className="bg-gray-800 rounded-xl p-4">
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {exec.kpis.some(k => k.score !== null) ? (
                    <span className={`text-sm font-bold px-2 py-0.5 rounded border ${RANK_COLORS[exec.rank]}`}>
                      {exec.rank}評価
                    </span>
                  ) : (
                    <span className="text-xs px-2 py-0.5 rounded bg-gray-700 text-gray-400 border border-gray-600">Q集計中</span>
                  )}
                </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {qMonths.map(mo => {
                ...
                const onTrack = monthStatuses.filter(s => s === 'on_track').length
                    ...
                    <div className="flex justify-center gap-1 mt-1">
                      {monthStatuses.map((s, i) => (
                        <div key={i} className={`w-2 h-2 rounded-full ${
                          ...
                          s === 'behind' ? 'bg-orange-400' : 'bg-gray-600'
                        }`} />
                      ))}
                    </div>
                    <div className="text-[9px] text-gray-500 mt-0.5">
                      {achieved + onTrack > 0 ? `${achieved}/${total}達成` : 'データなし'}
                    </div>
                  </div>
                )
              })}
                  ...
                  <tbody>
                    {kpi.monthlyProgress.map(mp => {
                      const achieveRate = mp.actual !== null && mp.target !== null && mp.target > 0
                        ...
                        : null
                      const style = STATUS_STYLES[mp.status]
                        ...
                        <tr key={mp.month} className={`border-b border-gray-700/30 ${isCurrent ? 'bg-blue-500/5' : ''}`}>
                          ...
                          <td className="py-1.5 px-1 text-right">
                            {editingKpi?.key === kpi.key && editingKpi?.month === mp.month ? (
                              ...
                              <span className={mp.actual !== null ? 'text-white font-medium' : 'text-gray-600'}>
                                {mp.actual !== null ? `${mp.actual.toLocaleString()}${kpi.unit}` : '—'}
                                {mp.autoValue !== null && mp.manualValue !== null && (
                          ...
                          <td className="py-1.5 px-1 text-right">
                            {!(editingKpi?.key === kpi.key && editingKpi?.month === mp.month) && (
                              <button
                                onClick={() => {
                                  setEditingKpi({ key: kpi.key, month: mp.month })
          ...
          <div className="bg-gray-800 rounded-xl p-4">
            ...
            <p className="text-[10px] text-gray-500 mb-2">全3ヶ月のデータが揃った時点でQ最終評価が確定します</p>
